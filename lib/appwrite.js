  ...
  const asset = {
    name:
      ...
      (fileType === 'video'
        ? `video-${Date.now()}.mp4`
        : `image-${Date.now()}.jpg`),
    type: file.mimeType ?? (fileType === 'video' ? 'video/mp4' : 'image/jpeg'),
    size: file.size ?? 0,
    uri: file.uri,
  };
...
export const deleteVideoPost = async (videoId) => {
  try {
    if (!videoId) throw new Error('deleteVideoPost: videoId is missing');
    ...
    console.log('deleteVideoPost error:', error);
